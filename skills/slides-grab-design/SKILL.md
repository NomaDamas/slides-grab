...
12. If motion materially explains sequence, accumulation, propagation, redistribution, or state change, author the complete final static state first. Mark the moving region with `data-motion-root` and start CSS animation only from `[data-motion-root].slides-grab-motion-active`; do not add per-slide lifecycle JavaScript. Keep Chart.js animation disabled.
...
- Keep motion source complete without animation; use only `data-motion-root` plus the public `slides-grab-motion-active` class and verify start, middle, final, reduced-motion, and static export states.
